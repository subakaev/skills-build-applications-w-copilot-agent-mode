...
function Workouts() {
  ...
  const loadWorkouts = useCallback(() => {
    ...
    fetch(endpoint)
      ...
      .catch((err) => {
        ...
        setError('Unable to load workouts from API.');
      })
  ...
  const filteredWorkouts = useMemo(() => {
    ...
    return workouts.filter((workout) =>
      JSON.stringify(workout).toLowerCase().includes(term)
    );
  }, [workouts, searchTerm]);
      ...
      <div className="card-body">
        <form className="row g-2 align-items-end mb-3" onSubmit={(event) => event.preventDefault()}>
          ...
          <div className="col-md-4 d-flex gap-2">
            <button type="button" className="btn btn-primary w-100" onClick={loadWorkouts}>
            ...
            <tbody>
              {filteredWorkouts.map((workout, index) => (
                <tr key={workout.id || workout._id || index}>
                  ...
                  <td>{workout.name || workout.title || `Workout ${index + 1}`}</td>
                  <td>{workout.duration || workout.level || workout.category || '-'}</td>
                  <td className="json-cell">{JSON.stringify(workout)}</td>
